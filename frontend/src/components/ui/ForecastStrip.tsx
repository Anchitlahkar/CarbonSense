  ...
  return (
    <Panel level={level} className={`w-full ${className}`} {...props}>
      <div className="flex items-center space-x-1.5 mb-2">
        ...
        <span className="text-[9px] font-bold text-text-muted/70 uppercase tracking-[0.1em] font-display">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {periods.map((period, index) => {
          ...
          return (
            <div 
              key={index}
              className="bg-bg-card/40 border border-white/[0.04] rounded-sm p-2 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between text-[8px] uppercase font-mono tracking-tighter text-text-subtle">
              ...
              <div className="flex items-baseline space-x-2 my-1">
                ...
                <span className="text-[9px] font-mono text-text-muted/60 lowercase">kg CO₂e</span>
                {savingsPercent > 0 && (
                  <span className="text-[8px] font-mono text-accent-green bg-accent-green/5 border border-accent-green/10 px-1 rounded-sm ml-auto font-bold">
                    -{savingsPercent}% path
                  </span>
                )}
              </div>
              ...
              <div className="space-y-1 text-[8px] font-mono border-t border-white/[0.03] pt-1.5 mt-1.5 uppercase tracking-tighter">
